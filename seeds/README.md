...
func CitiesSeeder(db *gorm.DB) error {
	...
	fmt.Printf("✓ Cities seeding completed: %d created, %d skipped\n", created, skipped)
	return nil
}
```
...
list_seeders() {
    ...
    echo -e "${GREEN}▶ cities${NC}           - Seed cities table"
...
func BooksSeeder(db *gorm.DB) error {
	fmt.Println("📝 Seeding books table...")
	...
	fmt.Println("✓ Books seeding completed")
	return nil
}
```
...
seeds/
├── README.md              # This file
├── jenis_buku_seed.go    # Example seeder
└── [your_seeder].go      # Add more seeders here
